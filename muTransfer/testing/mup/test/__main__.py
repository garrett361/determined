...
class SetBaseShapeCase(unittest.TestCase):
    ...
    def get_mlp_infshapes1(self):
        ...
        set_base_shapes(
            target_model, base_model, delta=delta_model, savefile=self.mlp_base_shapes_file
        )
...
class BackwardCompatibleCase(unittest.TestCase):
    def gen_model(self, arch, width, batchnorm=False, mup=True):
        if arch == "mlp":
            return generate_MLP(
                width=width, batchnorm=batchnorm, readout_zero_init=False, base_width=256, mup=mup
            )
        elif arch == "cnn":
            return generate_CNN(
                width=width, batchnorm=batchnorm, readout_zero_init=False, base_width=8, mup=mup
            )
        else:
            raise ValueError()
        ...
        for init_name, init in init_methods.items():
            ...
            for name in diff_names:
                with self.subTest(name=f"{name}, {init_name}"):
                    self.assertNotEqual((mup_params[name] - SP_params[name]).abs().sum().item(), 0)
            for name in same_names:
                with self.subTest(name=f"{name}, {init_name}"):
                    self.assertEqual((mup_params[name] - SP_params[name]).abs().sum().item(), 0)
...
class CoordCheckCase(unittest.TestCase):
    def test_MLP_CNN(self):
        combos = list(
            itertools.product(
                ["mlp", "cnn"], [True], [False, True], ["sgd", "adam"], init_methods.keys()
            )
        )
        ...
        for arch, mup, batchnorm, optimizer, init in combos:
            ...
            df = get_coord_data(
                models, train_loader, mup=mup, optimizer=optimizer, flatten_input=arch == "mlp"
            )
            df = df[df.module != ""]
...
class MLPTrainCase(unittest.TestCase):
    ...
    def test_init(self):
        ...
        for w in [64, 256]:
            self.assertLess(
                torch.abs(stds[(1024, 1, "weight")] - stds[(w, 1, "weight")])
                / stds[(1024, 1, "weight")],
                3e-3,
            )
            # for l in [1, 2]:
    ...
    def _test_train(self, opt):
        ...
        with self.subTest(name=f"{opt}, step 1"):
            self.assertTrue(
                loss[64][0] == loss[256][0] == loss[1024][0], {k: v[0] for k, v in loss.items()}
            )
        for t in [100, 200]:
            with self.subTest(name=f"{opt}, step {t+1}"):
                self.assertTrue(
                    loss[64][t] > loss[256][t] > loss[1024][t], {k: v[t] for k, v in loss.items()}
                )
...
class CNNTrainCase(unittest.TestCase):
    ...
    def test_init(self):
        ...
        for w in [8, 32]:
            self.assertLess(
                torch.abs(stds[(128, 0, "weight")] - stds[(128, 0, "weight")])
                / stds[(128, 0, "weight")],
                3e-3,
            )
        for name in names[:-1]:
            self.assertLess(
                torch.abs(stds[(128, 0, "bias")] - stds[(w, 0, "bias")]) / stds[(128, 0, "bias")],
                2e-1,
            )
    ...
    def _test_train(self, opt):
        ...
        with self.subTest(name=f"{opt}, step 1"):
            self.assertTrue(
                loss[8][0] == loss[32][0] == loss[128][0], {k: v[0] for k, v in loss.items()}
            )
        for t in [200]:
            with self.subTest(name=f"{opt}, step {t+1}"):
                losses = {k: v[t] for k, v in loss.items()}
                # print(losses)
                self.assertTrue(loss[8][t] > loss[32][t] > loss[128][t], losses)
