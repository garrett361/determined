...
init_methods = {k: partial(init_model, sampler=s) for k, s in samplers.items()}


def _generate_MLP(width, bias=True, mup=True, batchnorm=False, device="cpu"):
    mods = [
        Linear(3072, width, bias=bias, device=device),
        nn.ReLU(),
...
def get_train_loader(batch_size, num_workers=0, shuffle=False, train=True, download=False):
    ...
    trainset = datasets.CIFAR10(root="dataset", train=train, download=download, transform=transform)
