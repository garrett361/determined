...
def create_run_command(
    master_address: str, hostfile_path: Optional[str], override_args: List[str]
) -> List[str]:
    ...
    deepspeed_process_cmd.extend(override_args)
    deepspeed_process_cmd.append("--")
    print("DEEPSPEED_PROCESS_CMD TEST: ", deepspeed_process_cmd)
...
def main(override_args: List[str], script: List[str]) -> int:
    ...
    cmd = create_run_command(master_address, hostfile_path, override_args)
