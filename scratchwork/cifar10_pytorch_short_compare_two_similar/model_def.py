...
class FakeCifar(Dataset):
    def __init__(self, num_images: int = 10**4) -> None:
        self.num_images = num_images
        self.data = torch.randn(self.num_images, NUM_CHANNELS, IMAGE_SIZE, IMAGE_SIZE)

    def __len__(self) -> int:
        return self.num_images

    def __getitem__(self, idx: int):
        return self.data[idx], idx % NUM_CLASSES
...
class CIFARTrial(PyTorchTrial):
    def __init__(self, context: PyTorchTrialContext) -> None:
        ...
        logging.info(f"container_rank {info.container_rank}")
