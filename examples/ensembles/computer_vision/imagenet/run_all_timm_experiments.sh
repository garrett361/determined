...
python3 script_timm.py -m "$MASTER_URL" -nbm "$NUM_BASE_MODELS" -ne "$NUM_ENSEMBLES" -es naive naive_logits naive_geometric most_confident majority_vote -mc "$MODEL_CRITERIA" --no_safety_check --delete_unvalidated
python3 script_timm.py -m "$MASTER_URL" -nbm "$NUM_BASE_MODELS" -ne "$NUM_ENSEMBLES" -es naive_temp naive_logits_temp naive_geometric_temp most_confident_temp super_learner_probs super_learner_logits -mc "$MODEL_CRITERIA" --no_safety_check -e "$EPOCHS"
