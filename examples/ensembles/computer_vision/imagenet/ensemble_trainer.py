...
class EnsembleStrategy:
    ...
    The prob_fn takes in the vectorized model logits, with the model dimension last, and returns
    the probability generated by the entire ensemble.
    """

    def __init__(self, build: Callable, prob_fn: Callable) -> None:
        self.build = build
...
class EnsembleTrainer(nn.Module):
    ...
    def get_ensembled_probs(self, inputs: torch.Tensor) -> torch.Tensor:
        """Returns probabilties for the ensembled model."""
        logits = self(inputs)
        ensemble_probs = self._ensemble_strategies[self.ensemble_strategy].prob_fn(logits)
        if self.sanity_check:
            prob_sum_check = ensemble_probs.sum(dim=1)
    ...
    def validate_ensemble(self) -> None:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.val_loader):
                ...
                probs = self.get_ensembled_probs(inputs)
