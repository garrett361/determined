...
class TransformerStrategy(Strategy):
    """Transformer processing of logits."""
    ...
    def build(self) -> None:
        self.ensemble.transformer = ensemble_transformer.EnsembleTransformer(
            device=self.ensemble.device
        )
        self.ensemble.train_transformer()

    def pred(self, logits: torch.Tensor) -> torch.Tensor:
        # Need to transpose dimensions to the expected format.
        logits = logits.transpose(-1, -2)
        output_logits = self.ensemble.transformer(logits)
        ensemble_probs = output_logits.softmax(dim=-1)
        return ensemble_probs
>>>>>>> 54133b600 (starting to add transformers)
