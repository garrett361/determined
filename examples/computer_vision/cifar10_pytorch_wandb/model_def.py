...
class CIFARTrial(PyTorchTrial):
    def __init__(self, context: PyTorchTrialContext) -> None:
        ...
        # Assuming you made a `determined` project in your wandb account.
        self.exp_id = self.context.get_experiment_id()
        self.name = self.context.get_experiment_config()["name"]
        self.trial_id = self.context.get_trial_id()
        wandb.init(
            project="determined",
            config=self.hparams,
            group=str(self.exp_id),
            id=str(self.trial_id),
            name=self.name,
        )
    ...
    def train_batch(
        ...
        train_metrics = {"loss": loss, "train_error": 1.0 - accuracy, "train_accuracy": accuracy}
    ...
    def evaluate_batch(self, batch: TorchData) -> Dict[str, Any]:
        ...
        output = self.model(data)
        accuracy = accuracy_rate(output, labels)
        validation_metrics = {"validation_accuracy": accuracy, "validation_error": 1.0 - accuracy}
