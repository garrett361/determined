...
class ModelArguments:
    ...
    attn_impl: Optional[str] = field(
        default=None, metadata={"help": ("Specify the attention impelemntation to use.")}
    )
    data_files: Optional[str] = field(
        default=None, metadata={"help": {"data_files argument to load_dataset"}}
    )
...
def main(det_callback, tb_callback, model_args, data_args, training_args):
    ...
    if data_args.dataset_name is not None:
        ...
        raw_datasets = load_dataset(
            ...
            data_files=model_args.data_files,
        )
    ...
    if model_args.attn_impl:
        config.attn_config["attn_impl"] = model_args.attn_impl
    tokenizer_kwargs = {
        "cache_dir": model_args.cache_dir,
